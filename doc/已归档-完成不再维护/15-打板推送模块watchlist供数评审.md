...
| **已确认的供数缺口**（条件触发，须上线前处理） | **1 条**：早盘生成调度真空，KPL 数据晚到日 → 当日整日空 watchlist（执行侧只守仓、漏当日全部打板信号）。等级 SUPPLY_DEGRADING（非致命：不错买、不崩溃、优雅降级）。**已落地修复**：新增盘前补跑 cron 覆盖 8:56–9:15 真空窗（见 §2.1 整改记录）。 |
...
**整改记录（已落地）**：信号侧 `app/jobs/limit_up_push_jobs.py` 新增专用盘前补跑 cron `limit-up-premarket-catchup`（交易日 **9:00–9:14 每分钟**触发，复用幂等入口 `generate_and_push_limit_up_job`），覆盖整个 8:56–9:15 真空窗——KPL 在 8:57~9:14 任一时刻落地，下一补跑触发点必 ≤9:14，赶在执行侧 9:15 定稿前完成生成落表；**不改主轮询 8:31–8:56 既有节奏、不新增 8:30 前空轮询**；`coalesce + max_instances=1` 保证密集触发不堆叠、报告缓存与推送流水幂等不重复调 LLM/重复推送。已校验 cron 触发点（9:00,9:01,…,9:14）与三任务注册无误、ruff 通过。
**残留口径（须知，非本次最小修复范围）**：cron 密度只保证「在 9:15 前发起生成」，多阶段 LLM 生成本身有耗时——若 KPL 迟至 9:13–9:14 才落地，生成可能仍赶不上 9:15 定稿。要彻底根除须执行侧放宽定稿时点或缩短生成耗时（跨仓）。
...
| 优先级 | 事项 | 验收标准 | 依赖 |
|---|---|---|---|
| **P0（已落地）** | §2.1 早盘生成调度真空 | ✅ 已加 `limit-up-premarket-catchup` 补跑（交易日 9:00–9:14）覆盖真空窗；残留 LLM 耗时口径见 §2.1 整改记录。建议补该窗口回归用例 | 若进一步放宽执行侧 9:15 定稿，须与执行侧协同 |
