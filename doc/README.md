...
> 本文是整个项目（信号侧 + 执行侧）的单一入口（交易逻辑 / 架构 / 部署模块设计）；
> 还要做什么见 [`待办与上线验证清单.md`](待办与上线验证清单.md)，最近评审与修复见 [`08`](08-信号侧与执行侧量化交易代码评审报告.md)/[`09`](09-两侧代码修复计划与执行清单.md)，
> 实战战法见 [`06-A股龙头战法与追板操作指导手册.md`](06-A股龙头战法与追板操作指导手册.md)；历史开发设计/计划已移入 [`已归档-完成不再维护/`](已归档-完成不再维护/)（不再看）。
...
- **执行侧（Windows Server 2022）**：4 核 / 8GB / **≥60GB SSD**（≥100GB 省心）、国内低延迟到券商；miniQMT 常驻登录 + 程序化交易/分钟级行情权限。
  ...
  - 上线步骤/验证见 [`待办与上线验证清单.md`](待办与上线验证清单.md)；历史详细运维清单见 [已归档/07-QMT执行侧服务器部署运维清单](已归档-完成不再维护/07-QMT执行侧服务器部署运维清单.md)。
...
文档分四类（规则见仓库 `CLAUDE.md`）：① 本 README（项目现状）；② 待办清单（还要做什么）；③ 评审处理文档（处理中，处理完归档）；④ 已归档（完成存档、不再看）。

**① 现状 / 参考（README 同级，常看）**

| 文档 | 内容 |
|---|---|
| `README.md`（本文） | 项目总览：交易逻辑 + 架构 + 部署模块设计 |
| [`待办与上线验证清单.md`](待办与上线验证清单.md) | 还需处理 + 只能上生产/目标机实测验证的事项（活文档） |
| [`06-A股龙头战法与追板操作指导手册.md`](06-A股龙头战法与追板操作指导手册.md) | 实战战法与操作准则（人读手册） |

**② 最新评审处理文档（处理中，处理完后移入归档）**
...
针对本项目「真实投入资金」的属性，做了一次正式代码评审（[`08`](08-信号侧与执行侧量化交易代码评审报告.md)，166 条发现，确认 12 critical + 24 high + 63 medium），并据此完成修复（计划与逐项 commit 台账见 [`09`](09-两侧代码修复计划与执行清单.md)）。
...
- 回测显著性检验、默认含费净口径、LLM 概率校准、龙头打分全涨停池基准、role 分级消费等优化项见 [`09`](09-两侧代码修复计划与执行清单.md) 剩余项，及 [`待办与上线验证清单.md`](待办与上线验证清单.md)。
