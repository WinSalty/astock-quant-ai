...
| 实盘对接（miniQMT/xtquant + 调度任务计划 + 实测） | 🟡 **2026-06-18 晚 模拟上线配置完成（交易通道保持关）**：miniQMT 已装+登录（`userdata_mini=C:\qmtrun\userdata_mini` 纯英文）、`xtquant-250516.1.1` 接入 venv、`run_qmt.bat` 已填 `QMT_ACCOUNT_ID`/`QMT_MINI_PATH`；实测（只读零下单）xtdata 取真实快照 + xttrader connect rc=0/账户确认 + `build_real_engine` collect-only 装配 OK。**2026-06-24 §A 只读口径已实测确认**（A1–A7/A10 在生产 `xtquant_250516` 与代码占位值完全一致、全程零下单）；**2026-06-26 A4 竞价窗已核对**（集合竞价段 `get_full_tick` 能取到 `bidVol`，但发现竞价段盘口布局口径问题：`bidVol[0]`=虚拟匹配量、真实涨停封单在买侧残余 `bidVol[1]`，**启用竞价择时前须先定竞价段虚拟封单取数口径**，当前门控关无在线影响），见 [待办 §A](待办与上线验证清单.md) 与 [doc/33](33-生产部署核对与QMT执行侧口径实测确认-20260624.md)。**剩**：交易日手动 collect-only 实地观察 → 计划任务注册 → §A 残留项（A8 / A9 / A10 截断，需真实下单）随首批真实委托核对 → go/no-go 后才开 `KILL_SWITCH`。详见 [待办 §G](待办与上线验证清单.md) + 运维手册 §F |
...
| 文档 | 内容 |
|---|---|
| [`23-打板因子补充-watchlist契约1.2.0-设计与台账`](23-打板因子补充-watchlist契约1.2.0-设计与台账.md) | 信号侧 watchlist 契约补打板因子（封板时序 + 封单额口径统一 + 位置/强度，1.1.0→1.2.0）；执行侧消费接线列为待办（[`§D`](待办与上线验证清单.md)）。**未合并部署，处理完后归档**（其 P1-2 封单额 fd_amount 回退死代码已于 2026-06-21 修复，待重新部署，见 [`§D`](待办与上线验证清单.md)） |
| [`28-回测方向问题记录`](28-回测方向问题记录.md) | 第七轮评审剥离的回测口径问题（LLM 选股、回测样本有限）；**代码修复优先级低、可暂不处理**，活文档留存，待回测体系完善时一并处理 |

> **已归档（处理完移出本仓库，丢弃不再维护）**：第六轮 21/22（全链路评审+台账）、第七轮 24/25/26（执行侧/信号侧/联合独立评审）、27（修复顺序指南）、缺测改造 29/30（设计实现规格 + 遗留三项台账）已移入 [`../../../doc/已归档-完成不再维护/`](../../../doc/已归档-完成不再维护/)。**高层状态见 ③ 概要**；缺测改造口径变更（2026-06-21）：缺测「持仓卖出」(B3) 已下线、卖出交由实时盘口、缺测仅保留买入侧拦截，doc/30 项 1/2/3 已处理。
