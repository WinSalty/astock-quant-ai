...
> 实战战法见 [`06-A股龙头战法与追板操作指导手册.md`](06-A股龙头战法与追板操作指导手册.md)，生产部署/运维见本机运维手册 `/Users/salty/codeProject/ai/doc/Qmt生产服务器.txt`；历史开发设计/计划已移入 [`已归档-完成不再维护/`](已归档-完成不再维护/)（不再看）。
...
> ⚠️ 上线前务必先看 [`待办 §B 上线部署步骤`](待办与上线验证清单.md)——含两个必须的部署动作（信号侧跑迁移 0054、执行侧提供交易日清单文件）；评审与修复全貌见 [评审与修复状态概要](评审与修复状态概要.md)。
...
| [`评审与修复状态概要.md`](评审与修复状态概要.md) | 评审做了什么、修了什么、当前状态（持久概要，指向 08/09 详情） |
| [`06-A股龙头战法与追板操作指导手册.md`](06-A股龙头战法与追板操作指导手册.md) | 实战战法与操作准则（人读手册） |
...
| 文档 | 内容 |
|---|---|
| [`08-信号侧与执行侧量化交易代码评审报告`](08-信号侧与执行侧量化交易代码评审报告.md) | 两侧量化/供数代码正式评审（P0/P1/P2 + 优化空间 + 上线硬门槛）；明细见 [`08-评审发现明细.json`](08-评审发现明细.json) |
| [`09-两侧代码修复计划与执行清单`](09-两侧代码修复计划与执行清单.md) | 评审后修复的分批计划与逐项进度台账（含 commit 索引、待确认口径、部署须知） |
| [`10-信号侧打板报告与watchlist生成逻辑分析与优化建议`](10-信号侧打板报告与watchlist生成逻辑分析与优化建议.md) | 信号侧 LLM 打板报告/watchlist 链路分析：资金流向数据缺口、提示词细化、步骤逻辑与打分契约优化（27 条对抗式核验建议 + P0/P1/P2 与落地纪律） |
| [`11-量化交易代码第二轮独立评审报告`](11-量化交易代码第二轮独立评审报告.md) | 两侧交易代码独立复评（不依据旧评审文档、逐条对抗验证）：确认 92 条（6 P0 + 25 P1 + 39 P2 + 22 P3），按 11 条系统性根因主题归纳 + 分阶段上线硬门槛；明细见 [`11-评审发现明细.json`](11-评审发现明细.json) |
| [`13-量化交易系统第三轮独立评审报告`](13-量化交易系统第三轮独立评审报告.md) | 两侧交易/报告/watchlist 链路第三轮独立评审（多智能体扇出 + 逐条对抗式核验）：聚焦需处理的 P0/P1/P2 共 72 条（3 P0 + 14 P1 + 55 P2，已略去 69 条 P3 轻微/优化），含信号侧打板报告/watchlist 产出链路专项 + 分门槛上线建议；明细见 [`13-评审发现明细.json`](13-评审发现明细.json) |

**③ 已归档（完成存档·不再维护·无需再看）** → [`已归档-完成不再维护/`](已归档-完成不再维护/)（含 01–05 开发设计/计划、07 HTTP 交互/部署清单、旧 codex 评审稿）
