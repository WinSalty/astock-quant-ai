...
| 盘中卖出（止损/炸板/破位/连板了结/T+1 常规卖出） | ⚠️ **决策逻辑 + 下游状态机已落地且有单测**，但「实时盘口→`OrderBook` 派生 + `sell_books_provider` 注入」这段中间接线**尚未做**（`run.py:334 sell_books_provider=None`、`scheduler.py:142` provider 为 None 即 return → `run_sell_pass` 永不执行）→ **接线前买入后无法主动卖出，是当前唯一上线硬阻塞**；provider 接线/注入 + 4 个单帧字段（last_price/open_pct/封流比）可离线写 + fake 单测自证，但 `OrderBook` 中破位/炸板等 7 个跨帧盘口字段的完整保真依赖与买入侧同批的真机 tick 键名实测（见 [待办 §0/§A](待办与上线验证清单.md) 与 [状态概要](评审与修复状态概要.md)）|
