...
def build_order_book(ts_code: str, tick: Optional[dict], plan: Optional[PlanRow]) -> OrderBook:
    """单帧 tick + 计划行 → OrderBook（最小可用版，跨帧字段保守默认）。
    ...
    # —— 跨帧状态量：本最小版保守默认（OrderBook dataclass 默认即 False/0），**故意未填、非遗漏** ——
    # broke_board / below_support / volume_surge / near_close_weak / price_volume_diverge = False、open_times = 0。
    # 这些需保留 per-code 帧历史 + 真机 tick 键名（涨停价/均价/成交量量纲）才能保真，待阶段1 T1.2 实现；
    # 在此之前破位/炸板细粒度止损能力缺失，由 main._unit_stop_loss_breached 的【单帧浮亏止损】兜底（见 doc/16）。
    return ob
