...
- write-behind 后端（生产 SqliteQmtRepository）：upsert 只入队即返回，真正落盘在后台单写线程，异常【不会】在
  ...
  AsyncWriteQueue.flush_confirm + on_failure→Engine.on_storage_failure（停开新仓）+ is_persistently_failed 周期体检
  （执行-4/R10：周期 latch 只认持续性/间歇丢数据故障，瞬时 degraded 不永久冻结、由逐单 flush_confirm 把关）；
